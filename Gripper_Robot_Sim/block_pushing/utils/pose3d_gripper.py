...
class Pose3d_gripper(NoCopyAsDict):
    ...
    rotation_left: transform.Rotation
    rotation_right: transform.Rotation
    ...
    def vec7(self):
        ...
        self.rotation_left.as_quat(),
        self.rotation_right.as_quat()
    ])  # this gives [xL, yL, zL, xR, yR, zR, qxL, qyL, qzL, qwL, qxR, qyR, qzR, qwR]


    def serialize(self):
        return {
            "rotation_left": self.rotation_left.as_quat().tolist(),
            "rotation_right": self.rotation_right.as_quat().tolist(),
    ...
    def deserialize(data):
        return Pose3d_gripper(
            rotation_left=transform.Rotation.from_quat(data["rotation_left"]),
            rotation_right=transform.Rotation.from_quat(data["rotation_right"]),
            translation_left=np.array(data["translation_left"]),
            translation_right=np.array(data["translation_right"]),
        )

    def __eq__(self, other):
        return np.array_equal(self.rotation_left.as_quat(), other.rotation_left.as_quat()
        ) and np.array_equal(self.rotation_right.as_quat(), other.rotation_right.as_quat()
